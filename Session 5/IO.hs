...
sumTwo = do
  ...
  n2 <- readLn
  putStrLn ("The sum of both the numbers is " ++ show (n1 + n2 :: Int) ++ ".")
-- Task IO-5.
--
...
-- 1. directly by recursion on the integer/
-- 2. resuing exisiting funs usinf sequence , replicate
replicateM :: Int -> IO a -> IO [a]
replicateM i action
    | i <=0     = return []
    | otherwise = do
      x <- action
      xs <- replicateM (i-1) action
      return (x : xs)
...
sumMany :: IO ()
sumMany = do
  putStrLn "How many nos you want to add?"
  count <- readLn
  xs <- replicateM count readSingleNumber
  putStrLn ("The sum of all numbers is " ++ show (sum xs :: Int) ++ ".")

  where 
    readSingleNumber :: IO Int
    readSingleNumber = do
...
sumMany' = do
  putStrLn "How many nos you want to add?"
  count <- readLn
  xs <- mapM (\ i -> readSingleNumber count i) [1 .. count]
  putStrLn ("The sum of all numbers is " ++ show (sum xs :: Int) ++ ".")

  where 
    readSingleNumber :: Int -> Int -> IO Int
    readSingleNumber total current = do
      putStrLn ("enter number " ++ show current ++ " of " ++ show total ++ ".")
      readLn
